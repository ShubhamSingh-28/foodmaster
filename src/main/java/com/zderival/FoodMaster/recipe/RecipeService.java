...
public class RecipeService {
    ...
    public Recipe getRecipeInformation(int id){
        Recipe recipe = restClient.get()
                ...
                .retrieve()
                .body(Recipe.class);
        return processRecipeDetails(recipe);
    }

    private Recipe processRecipeDetails(Recipe recipe) {
        if (recipe == null) return null;
        Nutrition nutrition = new Nutrition();
        if (recipe.getSpoonacularNutrition() != null && recipe.getSpoonacularNutrition().getNutrients() != null) {
    ...
    public List<Recipe> getRecipes(List<String> ingredients){
        ...
        // Step 2: Use bulk retrieval to avoid N+1 requests
        String ids = searchResults.stream()
                .map(result -> String.valueOf(result.getId()))
                .collect(Collectors.joining(","));

        List<Recipe> recipes = restClient.get()
                .uri("https://api.spoonacular.com/recipes/informationBulk?ids={ids}&includeNutrition=true&apiKey={apiKey}",
                        ids, spoonacular_api_key)
                .retrieve()
